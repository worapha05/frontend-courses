...
export function useDataGrid<T extends Record<string, unknown>>(options: {
  ...
  function toggleSort(key: string, multi = false) {
    ...
    if (!multi) {
      if (existing) {
        sorts.value = [{ key, dir: existing.dir === 'asc' ? 'desc' : 'asc' }];
      } else {
  ...
  function exportCsv(filename = 'export.csv') {
    ...
    const cols = options.searchKeys.map(String);
    // include common numeric/meta cols if present
    const headerKeys = Object.keys(sorted.value[0] ?? {});
    const keys = headerKeys.length ? headerKeys : cols;
    const lines = [
      keys.join(','),
